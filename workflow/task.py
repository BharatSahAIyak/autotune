...
from .models import Workflows, Examples, WorkflowConfig
from .pydantic_models import QAResponse, QAPair


async def call_llm_generate(prompt_text, workflow, config):
    ...
    json_output_parser = JsonOutputParser(pydantic_object=QAPair)
...
def parse_and_save_examples(workflow, response):
    try:
        qa_response = QAResponse.model_validate(response)
...
async def generate_or_refine(workflow_id, refine=False):
    workflow = await sync_to_async(Workflows.objects.get)(pk=workflow_id)
    num_samples = settings.LLM_GENERATION_NUM_SAMPLES
    config = get_object_or_404(WorkflowConfig, name=workflow.workflow_type)

    prompt_text = await construct_prompt(workflow, config, refine, num_samples)
    response = await call_llm_generate(prompt_text, workflow, config)
    if response:
        await parse_and_save_examples(workflow, response)
    return response


def construct_prompt(workflow, config, refine=False, num_samples=20):
    """
