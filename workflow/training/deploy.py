...
def deploy_model(self, request_data):
    ...
    try:
        ...
        logger.info(f"Downloaded model from Hugging Face Hub: {finetuned_model_id}")
        push_to_hub(
            folder_path=model_path, repo_id=deployment_model_id, repo_type="model"
        )
        logger.info(f"Pushed model to Hugging Face Hub: {deployment_model_id}")
...
def run_github_workflow(
    ...
    while (
        repo.get_workflow_runs(
            status="in_progress",
            created=">="
            + (now - datetime.timedelta(seconds=5)).strftime("%Y-%m-%dT%H:%M:%S+00:00"),
        ).totalCount
        == 0
    ):
        pass
